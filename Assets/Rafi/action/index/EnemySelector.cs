...
public class EnemySelector : MonoBehaviour
{
    ...
    [SerializeField]
    private Transform selectedEnemy; // Display selected enemy in Inspector
    ...
    void Start()
    {
        ...
        if (enemies.Count > 0)
        {
            currentIndex = Random.Range(0, enemies.Count);
    ...
    void FixedUpdate()
    {
        if (selectedEnemy != null && !rotationStopped)
        {
    ...
    {
        Collider selectedCollider = selectedEnemy.GetComponent<Collider>();
    ...
    {
        Vector3 directionToEnemy = selectedEnemy.position - transform.position;
        ...
        float distanceToStopper = Vector3.Distance(selectedEnemy.position, stopperObject.transform.position);
